...
$('#buy').click( function(){carro.buyCasas()})
 ...
 `
    ...
    nuevoDiv.getElementsByClassName('less')[0].addEventListener('click', function (event) {
        if (casaCarrito.cantidad > 1) {
            ...
            casaCarrito.stock++
            nuevoDiv.querySelector(".quantity").textContent = `x ${casaCarrito.cantidad}`
            nuevoDiv.querySelector(".precio").textContent = `x ${casaCarrito.cantidad * casaCarrito.precio}`
            actualizar()
        } else return
    })
    ...
    nuevoDiv.getElementsByClassName('quantity')[0].innerText = `X${casaCarrito.cantidad}`
}
